...
export const getBaseRotations = (
  rotations: ReadonlyArray<RotationSchema>,
  email: string
): ReadonlyArray<RotationSchema> => {
...
const makeProgram = (
  ...
  (readonly RotationSchema[] | [readonly RotationSchema[], number])[],
  ParseError | Error,
  never
> => {
  const program = pipe(
    ...
    Effect.map((r) => {
      const [base, final] = [
        r.data.baseTimeline.rotations,
        r.data.finalTimeline.rotations,
      ];
...
export const showOnCallAction = async (email: string) => {
  ...
  try {
    const [base, [final, tokenSpesi]] = (await Effect.runPromise(
      makeProgram(email)
    )) as [
      ReadonlyArray<RotationSchema>,
      [ReadonlyArray<RotationSchema>, number]
    ...
    console.log(
      `Token Rimasti: ${JSON.stringify(TOKEN - tokenSpesi, null, 2)} \n\r`
    );
  } catch (error) {
